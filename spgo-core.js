...
// ============================================================
// GUEST CAPTCHA — Cloudflare Turnstile
// Portals call showGuestCaptcha() on their guest button.
// guestLogin() is called only after CAPTCHA passes.
// To override the modal appearance, override showGuestCaptcha()
// in the portal's own <script> block.
// Turnstile site key: replace TURNSTILE_SITE_KEY with your key
// from https://dash.cloudflare.com → Turnstile
// ============================================================
const TURNSTILE_SITE_KEY = '0x4AAAAAADQ6Oa4EQjYNz1Bf';

function showGuestCaptcha() {
  ...
  // Inject modal once
  if (!document.getElementById('guest-captcha-modal')) {
    const modal = document.createElement('div');
    modal.id = 'guest-captcha-modal';
    modal.style.cssText = 'display:none; position:fixed; inset:0; z-index:99999; background:rgba(0,0,40,0.92); flex-direction:column; align-items:center; justify-content:center; padding:24px;';
    modal.innerHTML = `
      <div style="background:#000073; border:1px solid rgba(0,191,255,0.3); border-radius:16px; padding:28px 24px; width:100%; max-width:340px; display:flex; flex-direction:column; align-items:center; gap:18px;">
        <div style="font-family:'Cinzel',serif; font-size:14px; color:#00BFFF; letter-spacing:0.12em; text-align:center;">Continue as Guest</div>
        <div style="font-family:'Crimson Pro',serif; font-size:13px; color:rgba(255,255,255,0.6); text-align:center; font-style:italic;">Please confirm you're human.</div>
        <div id="guest-turnstile-widget"></div>
        <div id="guest-captcha-status" style="font-family:'Crimson Pro',serif; font-size:12px; color:rgba(255,100,100,0.8); text-align:center; min-height:16px;"></div>
        <button onclick="closeGuestCaptcha()" style="background:none; border:none; color:rgba(255,255,255,0.4); font-family:'Cinzel',serif; font-size:11px; cursor:pointer; letter-spacing:0.1em;">✕ Cancel</button>
      </div>
    `;
    document.body.appendChild(modal);
  }
  ...
  // Show modal
  const modal = document.getElementById('guest-captcha-modal');
  modal.style.display = 'flex';
  document.getElementById('guest-captcha-status').textContent = '';

  // Render Turnstile widget (wait for script to load if needed)
  function renderWidget() {
    const container = document.getElementById('guest-turnstile-widget');
    if (!container) return;
    container.innerHTML = ''; // reset in case of re-open
    if (window.turnstile) {
      window.turnstile.render(container, {
        sitekey: TURNSTILE_SITE_KEY,
        theme: 'dark',
        callback: function(token) {
          // Token received — CAPTCHA passed, proceed to guest login
          closeGuestCaptcha();
          guestLogin();
        },
        'error-callback': function() {
          document.getElementById('guest-captcha-status').textContent = 'Verification failed. Please try again.';
        }
      });
    } else {
...
function closeGuestCaptcha() {
  const modal = document.getElementById('guest-captcha-modal');
  if (modal) modal.style.display = 'none';
  // Reset widget so it's fresh next time
  const container = document.getElementById('guest-turnstile-widget');
